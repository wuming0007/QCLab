from dev.common import QList, QOption, QReal, VisaDriver
...
class Driver(VisaDriver):
    ...
    def get_sequence_list(self):
        ...
        for i in range(slist_size):
            ret.append(self.handle.query("SLIS:NAME? %d" % i).strip("\"\n '"))
        return ret
    ...
    def create_waveform(self, name, length, format='REAL'):
        ...
        if name in self.waveform_list:
            return
        self.handle.write('WLIS:WAV:NEW "%s",%d,%s;*WAI;' %
                          (name, length, format))
        self.waveform_list = self.get_waveform_list()

    def remove_waveform(self, name=None, all=False):
        if all:
            self.handle.write('WLIS:WAV:DEL ALL; *WAI;')
            self.waveform_list.clear()
        elif name not in self.get_waveform_list():
            return
        else:
            self.handle.write('WLIS:WAV:DEL "%s"; *WAI;' % name)
            self.waveform_list = self.get_waveform_list()

    def get_waveform_length(self, name):
        size = int(self.handle.query('WLIS:WAV:LENGTH? "%s"' % name))
        return size

    def use_waveform(self, name, ch=1, type=None):
        '''type: I or Q'''
        if type is not None:
            self.handle.write('SOUR%d:CASS:WAV "%s",%s' % (ch, name, type))
        else:
            self.handle.write('SOUR%d:CASS:WAV "%s"' % (ch, name))
    ...
    def get_current_asset(self, ch=1):
        current_type = self.handle.query('SOUR%d:CASS:TYPE?' % ch)
        current_asset = self.handle.query('SOUR%d:CASS?' % ch)
        return current_type, current_asset

    def update_waveform(self, points, name='ABS', IQ='I', start=0, size=None):
        w_type = self.handle.query('WLIS:WAV:TYPE? "%s"' % name).strip()
        if w_type == 'REAL':
    ...
    def create_sequence(self, name, steps, tracks=1):
        if name in self.sequence_list:
            return
        self.handle.write('SLIS:SEQ:NEW "%s", %d, %d; *WAI;' %
                          (name, steps, tracks))
        self.sequence_list = self.get_sequence_list()

    def remove_sequence(self, name=None, all=False):
        if all:
            self.handle.write('SLIS:SEQ:DEL ALL; *WAI;')
            self.sequence_list.clear()
        elif name not in self.sequence_list:
            return
        else:
            self.handle.write('SLIS:SEQ:DEL "%s"; *WAI;' % name)
            self.sequence_list = self.get_sequence_list()
    ...
    #         self.handle.write('SLIS:SEQ:STEP%d:TASS:SEQ "%s","%s"' % (step, name, sub_name))
    #     else:
    #         for i, wav in enumerate(sub_name):
    #             self.handle.write('SLIS:SEQ:STEP%d:TASS%d:WAV "%s","%s"' % (step, i+1, name, wav))
    #     self.handle.write('SLIS:SEQ:STEP%d:WINP "%s", %s' % (step, name, wait))
    #     self.handle.write('SLIS:SEQ:STEP%d:GOTO "%s", %s' % (step, name, goto))
    #     self.handle.write('SLIS:SEQ:STEP%d:RCO "%s", %s' % (step, name, repeat))
    #     if jump is not None:
    #         self.handle.write('SLIS:SEQ:STEP%d:EJIN "%s", %s' % (step, name, jump[0]))
    #         self.handle.write('SLIS:SEQ:STEP%d:EJUM "%s", %s' % (step, name, jump[1]))

    def use_sequence(self, name, ch=1, track=1, type=None):
        '''type: I or Q'''
        if type is not None:
            self.handle.write('SOUR%d:CASS:SEQ "%s", %d, %s' %
                              (ch, name, track, type))
        else:
            self.handle.write('SOUR%d:CASS:SEQ "%s", %d' % (ch, name, track))
        self.handle.write('*WAI;')
