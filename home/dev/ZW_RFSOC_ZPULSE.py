...
# try:
from dev.common.ZWD_DDS import soc_zpulse as fpgadev
# except:
...
class Driver(BaseDriver):
    ...
    support_models = ['ZW_RFSOC_ZPULSE']
    ...
    quants = [
        ...
        QString('Nyquist', value='normal', ch=1),
    ...
    def __init__(self, addr, **kw):
        ...
        self.model = 'ZW_RFSOC_ZPULSE_24'
        self.handle = fpgadev()
        self.srate = 8e9

    def open(self):
        self.triggerClose()
        # self.setValue('SamplingMode', 8000)
    ...
    def write(self, name: str, value, **kw):
        ...
        if name in ['Waveform', 'Output', 'Nyquist', 'Offset']:
    ...
    def set_DA(self, name, value, ch):
        ...
        if name in ['Waveform']:
            ...
            if isinstance(value, Waveform):
                # value <<= (round(self.config['TriggerDelay'][ch]['value']*self.srate)/self.srate)
                # value.start, value.stop = 0, min(self._intrinsic_waveform_length, self.config['PointNumber'][ch]['value']*self.adc_srate*1.5)
            ...
            if value <= 1 and value >= -1:
                ...
                self.handle.dac_offset(
                    ch - 1, np.round(self.handle.dc1v * value + self.handle.offset[ch - 1]))
            else:
    ...
    def writeWaveform(self, data, ch=1, trigger_delay=0, continuous=False):
        ...
        data = np.round(data * self.handle.dc1v)
