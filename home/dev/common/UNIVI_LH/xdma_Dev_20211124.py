...
#                     self.Board_marknum['DC_BoardNumID']=zz
    ...
    def fpga_dma_write(self,bus_id,addr,data):
        size = 4096
        # print('DMA A addr = ' + str(hex(addr)))

        # fp = open('dma_addr.txt','a+')
        # waddr = np.uint64(addr)
        # fp.write(str(hex(addr)))
        # fp.write('\n')
        # fp.close()
        ...
        while 1:
            dma_write_state = pcie_fpga_dma_write(bus_id,addr,data,size)
...
#                        self.fpga_dma_write(board_id,data_addr,databuf)
                        ...
                        data_buf = np.int16(data_buf[:2048])
                        
                        pbuf.raw = data_buf
                        data_buf_wr = ctypes.cast(ctypes.addressof(pbuf), ctypes.POINTER(ctypes.c_short))
                        self.fpga_dma_write(board_id,data_addr,data_buf_wr)
                        
                        data_addr = data_addr + 0x1000
                        data_buf = np.int16(data_buf[2048:4096])
                        ...
                        data_addr = data_addr + 0x1000
                        data_buf = np.int16(data_buf[4096:6144])
                        ...
                        data_addr = data_addr + 0x1000
                        data_buf = np.int16(data_buf[6144:8192])
...
#                     return {}
    ...
    def query_demo_state(self, channelnum, slot):
        """22-02-11 LZY 询问解模完成状态,True表示采集完成"""
        state = self.readReg(0xc01,slot)
        is_complete = bool((state >> (channelnum-1)) & 1)
        return is_complete

    def get_demo_save_data(self,channelnum,data_len,slot=''):
        """22-02-11 LZY 增加询问解模完成状态"""
        for board_id in range(self.board_number):
            if slot == self.BoardInfo[str(board_id)][1]:
                self.boardtypeflag = self.BoardInfo[str(board_id)][0]
                if self.BoardInfo[str(board_id)][0]=='2DA2AD':
                    if channelnum>20:
                        ...
                        # 22-02-10 yuhai  
                        # state = self.readReg(0xc01,slot)
                        # if ((state >> (channelnum-1)) & 1) == 0:
                        #     return [],[],False
                        is_complete = self.query_demo_state(channelnum, slot)
                        time0=time.time()
                        while not is_complete:
                            time.sleep(0.005)
                            time1=time.time()
                            if time1-time0>10:
                                raise Exception('get_demo_save_data timeout!')
                            is_complete = self.query_demo_state(channelnum, slot)
...
#                        print('wave too largh')
                        ...
                        self.fpga_dma_write(board_id,dac_data_addr,data_buf_wr)
            ...
            if slot == self.BoardInfo[str(board_id)][1]:
                ...
                for wave_id in range(len(dac_data)):
                    ...
                    for j in range(len(dac_data[wave_id])):
                        ...
                        data_buf = np.int16(np.zeros(2048))
                        data_buf[:data_len] = np.int16(dac_data[wave_id][j])
                        pbuf.raw = data_buf
...
#                     while ((self.readReg(0x4001,slot)>>16) & dac_update_state) != dac_update_state:  
#                         print(hex((self.readReg(0x4001,slot)>>16) & 0xfff),hex(dac_update_state),slot)
#                         time.sleep(0.001)
#                 elif self.BoardInfo[str(board_id)][0]=='2DA2AD':
            ...
            if slot == self.BoardInfo[str(board_id)][1]:
                ...
                if self.BoardInfo[str(board_id)][0]=='2DA2AD':
                    if (read_data_len*num > 1024*1024*1024) | (read_data_len>=2**20):
                        ...
                        # 22-02-11 LZY 优化
                        is_complete = self.readReg(0x405,slot)
                        time0=time.time()
                        while not is_complete:
                            time.sleep(0.005)
                            time1=time.time()
            ...
            if slot == self.BoardInfo[str(board_id)][1]:
                ...
                if self.BoardInfo[str(board_id)][0]=='Clock':
                    ...
                    for i in range(int(data_len/4096)):
                        ...
                        self.fpga_dma_write_bysize(board_id,addr_i,data_buf_wr,4096)
                        addr_i += 4096
