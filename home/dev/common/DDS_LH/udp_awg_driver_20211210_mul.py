...
userDLL_path = '.\\UdpDLL_20210822.dll'
...
class fpgadev(object):
    ...
    adc_chennel = devip.adc_chennel
    dac_chennel = devip.dac_chennel
    
    def __init__(self):   
        print("start\n")
        self.dll=ctypes.WinDLL(userDLL_path)
        
        for i in range(len(fpgadev.adc_chennel)):
            fpgadev.adc_chennel[i][2] = self.socket_connect()
        for i in range(len(fpgadev.dac_chennel)):
            fpgadev.dac_chennel[i][2] = self.socket_connect()
...
#                        print(1)
    ...
    def dac_trigger_close(self):
        ...
        server_Addr = (devip.clock_board[0][0],devip.clock_board[0][1])
        clk_tcp_client_socket.connect(server_Addr)
...
#        clk_tcp_client_socket.send(tcp_buffer)
        ...
        self.system_sampling_cmd_send(devip.clock_board[0],0x06,sampling)
        time.sleep(2)
        
        self.dac_trigger_ctrl(devip.clock_board[0],0,4,10,0)
        
        time.sleep(0.5)
        
        self.system_sampling_cmd_send(devip.clock_board[0],0x07,sampling)
        time.sleep(0.5)
        ...
        self.system_sampling_cmd_send(devip.clock_board[0],0x04,sampling)
        
        for i in range(len(devip.clock_board)-1):
        
            self.system_sampling_cmd_send(devip.clock_board[i+1],0x03,sampling)
            time.sleep(5)
            
            self.dac_trigger_ctrl(devip.clock_board[i],0,4,10,0)
            
            time.sleep(0.5)
           
            self.system_sampling_cmd_send(devip.clock_board[i+1],0x07,sampling)
            self.dac_trigger_ctrl(devip.clock_board[i+1],1,10,0,0)
            time.sleep(0.5)
            ...
            self.system_sampling_cmd_send(devip.clock_board[i+1],0x04,sampling)
        ...
        for i in range(len(self.dac_board)):
            self.system_sampling_cmd_send(devip.dac_board[i],0x01,sampling)

        for i in range(len(self.adc_board)):
            self.system_sampling_cmd_send(devip.adc_board[i],0x01,sampling)
        ...
        for i in range(len(devip.dac_board)):
            self.system_sampling_cmd_send(devip.dac_board[i],0x02,sampling)

        for i in range(len(devip.adc_board)):
            self.system_sampling_cmd_send(devip.adc_board[i],0x02,sampling)
...
#        self.dac_trigger_ctrl(self.clock_board[0],0,1,10,0)
        ...
        for i in range(len(devip.dac_board)):
            self.system_sampling_cmd_send(devip.dac_board[i],0x03,sampling)
            
        for i in range(len(devip.adc_board)):
            self.system_sampling_cmd_send(devip.adc_board[i],0x03,sampling)
...
#        time.sleep(0.5)
    ...
    def dac_Nyquist_cfg(self,dac_chennel_in,Nyquist):
        ...
        for i in range(len(devip.dac_chennel)):
            if devip.dac_chennel[i] == dac_chennel_in:
                chennal_id = i%4
                dac_board_id = devip.dac_board[i//4]
                break;
    ...
    def dac_updata(self,dac_chennel,dac_data):
    ...
    def dac_point_updata(self,dac_chennel,data_point):
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 1,dac_en = 0)
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 0,dac_en = 0)
        time.sleep(0.0005)
