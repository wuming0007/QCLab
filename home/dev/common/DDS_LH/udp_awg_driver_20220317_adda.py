...
#userDLL_path="D:\\cl\\Socket\\Socket_C_PLUS_DLL\\UdpDLL.dll"
from .udp_awg_ip_addr_20220317 import devip
import random
...
class fpgadev(object):
    ...
    def __init__(self,dev_ip):   
        ...
        self.dev_id = devip(dev_ip)
        
        for i in range(len(self.dev_id.ad)):
            self.dev_id.ad[i][2] = self.socket_connect()
        for i in range(len(self.dev_id.da)):
            self.dev_id.da[i][2] = self.socket_connect()
...
#                        print(1)
    ...
    def read_reg(self,slot_dac,reg_addr):
        ...
        pack_num = random.randrange(0,2**8-1)
        re_recv = 0
        while 1:
            if re_recv == 0:
            ...
            udp_data_rx,recv_len = self.udp_recv(slot_dac,4096)
            data_buf_i = ctypes.POINTER(ctypes.c_uint8)(udp_data_rx)
            pack_num_recv = data_buf_i[1]
#            print(pack_num,pack_num_recv)
#            print('read_reg',recv_len)
            if recv_len == -1:
                re_recv = 0
            elif recv_len != -1 and recv_len != 22:
                re_recv = 1
            elif pack_num != pack_num_recv:
                re_recv = 1
            elif recv_len == 22:
                break
...
#            print('write_reg',recv_len)
    ...
    def dac_trigger_ctrl(self,trigger_source,trigger_us,trigger_num,trigger_continue):
        ...
        clk_board = self.dev_id.clock_board[0]
    ...
    def dac_trigger_ctrl_ins(self,clk_board,trigger_source,trigger_us,trigger_num,trigger_continue):
        #trigger_source 0 为内部触发，1为外部触发
        #trigger_us 触发周期单位us
        #trigger_num 触发次数，当trigger_continue为1时无效
        #trigger_continue 1 为连续触发模式，此时触发次数无效；0 为按触发次数触发。
    ...
    def dac_trigger_close(self):
        ...
        server_Addr = (self.dev_id.clock_board[0][0],self.dev_id.clock_board[0][1])
        clk_tcp_client_socket.connect(server_Addr)
    ...
    def system_sampling(self,sampling):
        ...
        self.system_sampling_cmd_send(self.dev_id.clock_board[0],0x06,sampling)
        time.sleep(2)
        
        self.dac_trigger_ctrl_ins(self.dev_id.clock_board[0],0,6.4,10,0)
        
        time.sleep(0.5)
        
        self.system_sampling_cmd_send(self.dev_id.clock_board[0],0x07,sampling)
        time.sleep(0.5)
        ...
        self.system_sampling_cmd_send(self.dev_id.clock_board[0],0x04,sampling)
        
        time.sleep(0.5)
        
        for i in range(len(self.dev_id.clock_board)-1):
#            print(self.dev_id.clock_board[i+1])
            self.system_sampling_cmd_send(self.dev_id.clock_board[i+1],0x03,sampling)
            time.sleep(2)
            self.dac_trigger_ctrl_ins(self.dev_id.clock_board[i],0,6.4,10,0)
            
            time.sleep(1)
            
            self.system_sampling_cmd_send(self.dev_id.clock_board[i+1],0x07,sampling)
#            time.sleep(0.1)
            self.dac_trigger_ctrl_ins(self.dev_id.clock_board[i+1],1,4,10,0)
#            time.sleep(0.1)
            ...
            self.system_sampling_cmd_send(self.dev_id.clock_board[i+1],0x04,sampling)
            time.sleep(0.5)
            
        time.sleep(5)
        
        for i in range(len(self.dev_id.clock_board)-1):
            self.dac_trigger_ctrl_ins(self.dev_id.clock_board[i+1],1,10,0,0)

        time.sleep(0.5)
        
        
        for i in range(len(self.dev_id.board)):
            self.system_sampling_cmd_send(self.dev_id.board[i],0x01,sampling)
        ...
        time.sleep(0.5*len(self.dev_id.clock_board))
        
        for i in range(len(self.dev_id.board)):
            self.system_sampling_cmd_send(self.dev_id.board[i],0x02,sampling)
...
#        time.sleep(0.5)
#        
#        self.dac_trigger_ctrl_ins(self.clock_board[0],0,1,10,0)
        
        time.sleep(0.5*len(self.dev_id.clock_board))
        
        for i in range(len(self.dev_id.board)):
            self.system_sampling_cmd_send(self.dev_id.board[i],0x03,sampling)
            
        time.sleep(10)
#        self.dac_trigger_ctrl_ins(self.clock_board[0],0,4,10,0)
#        time.sleep(0.5)


      
        
    def dac_Nyquist_cfg(self,dac_chennel_in,Nyquist):
        #切换频域
        #dac_chennel_in 为DAC通道数,从1开始
        #Nyquist 为DAC工作模式，0 为正常模式；1 为mix模式，适合高频段
        dac_chennel_in = (dac_chennel_in//4)*2+dac_chennel_in%2
        if dac_chennel_in > (len(self.dev_id.da)-1) or dac_chennel_in < 0:
            print('未找到dac通道：',dac_chennel_in)
            return
        
        chennal_id = dac_chennel_in%4
        dac_board_id = self.dev_id.board[dac_chennel_in//4]
        clk_tcp_client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    def dac_updata(self,dac_chennel_num,dac_data):
        #dac_chennel_num为dac通道号，从1开始
        dac_chennel_num = (dac_chennel_num//4)*2+dac_chennel_num%2
        if dac_chennel_num > (len(self.dev_id.da)-1) or dac_chennel_num < 0:
            print('未找到dac通道：',dac_chennel_num)
            return
        
        dac_chennel = self.dev_id.da[dac_chennel_num]
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 1,dac_en = 0)
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 0,dac_en = 0)
#        time.sleep(0.0005)
        dac_data_point_bufe = self.write_dac_data(dac_chennel,dac_data)
        
        self.dac_data_point(dac_chennel,dac_data_point_bufe)
    
    
    def dac_point_updata(self,dac_chennel_num,data_point):
        #dac_chennel_num为dac通道号，从1开始
        dac_chennel_num = (dac_chennel_num//4)*2+dac_chennel_num%2
        if dac_chennel_num > (len(self.dev_id.da)-1) or dac_chennel_num < 0:
            print('未找到dac通道：',dac_chennel_num)
...
#        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 0,dac_en = 0)
#        time.sleep(0.0005)
        self.dac_replay_point(dac_chennel,data_point)
        
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 0,dac_en = 1)

    def dac_ch_close(self,dac_chennel):
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 1,dac_en = 0)
        self.dac_ch_ctrl(slot_dac = dac_chennel,dac_reset = 0,dac_en = 0)
...
#        print(read_data_len,len(recv_data_bufe_o),len(recv_data_bufe_o[0]),len(recv_data_bufe_o[1]))
#        for i in range(len(recv_data_bufe_o[0])):
#            print(0,len(recv_data_bufe_o[0][i]))
#            print(1,len(recv_data_bufe_o[1][i]))
    ...
    def adc_modle_reset(self,adc_chennel_num,modle_reset):
        ...
        adc_chennel_num = (adc_chennel_num//4)*2+adc_chennel_num%2
        if adc_chennel_num > (len(self.dev_id.ad)-1) or adc_chennel_num < 0:
            print('未找到adc通道：',adc_chennel_num)
            return
        
        adc_chennel = self.dev_id.ad[adc_chennel_num]
        dac_ctrl_reg = []
        ...
        self.write_reg(adc_chennel,adc_reg_addr,dac_ctrl_reg)
    ...
    def rd_adc_data_ctrl(self,adc_chennel_num,modle_en,trigger_delay,times,save_len):
        ...
        self.adc_save_ctrl(adc_chennel,0,modle_en,trigger_delay,times,save_len)
    ...
    def rd_adc_data(self,adc_chennel_num,times,save_len):
        ...
        save_state_i = self.read_reg(adc_chennel,0x10000001)
#        print(save_state_i)
        if (save_state_i[2]&1) == 0:
            ...
            adc_data = self.udp_rd_adc_data(adc_chennel,0,read_data_len)
...
#        self.adc_modle_reset(slot_dac,1)
        ...
        self.adc_mul_ctrl(adc_chennel,0,modle_en,modle_num,trigger_delay,times,save_len)
#        self.adc_modle_reset(slot_dac,0)
    ...
    def rd_adc_mul_data(self,adc_chennel_num,modle_num,times):
        ...
        save_state_i = self.read_reg(adc_chennel,0x10000003+modle_num)
#        print(save_state_i)
        if (save_state_i[2]&1) == 0:
            read_data_len = -1
            return [],read_data_len
        else:
            adc_data = self.udp_rd_muldata(adc_chennel,modle_num,times)
            read_data_len = times
    ...
    def adc_mul_data_wr(self,adc_chennel_num,modle_num,generate_data):
        ...
        self.adc_uram_wr(adc_chennel,generate_data[0],start_addr)
...
if __name__=='__main__':
    dev=fpgadev()
#    devip = awg_ip.dev_ip
#    for i in range(len(devip.adc_chennel)):
#        devip.adc_chennel[i][2] = dev.socket_connect()
#    for i in range(len(devip.dac_chennel)):
#        devip.dac_chennel[i][2] = dev.socket_connect()