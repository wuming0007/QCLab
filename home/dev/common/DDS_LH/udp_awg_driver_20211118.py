...
#        clk_tcp_client_socket.send(tcp_buffer)
        ...
        self.system_sampling_cmd_send(self.clock_board[1], 0x03, sampling)
        time.sleep(5)
        ...
        self.system_sampling_cmd_send(self.clock_board[1], 0x07, sampling)
        self.dac_trigger_ctrl(self.clock_board[1], 1, 10, 0, 0)
        time.sleep(0.5)
        ...
        self.system_sampling_cmd_send(self.clock_board[1], 0x04, sampling)
        time.sleep(5)
...
#        self.dac_trigger_ctrl(self.clock_board[0],0,1,10,0)
        ...
        while(recv_times_cnt < recv_times):
            ...
            if recv_len != -1:
                ...
                udp_rx_data_bufe.append(
                    np.int16(data_buf_i[0:int(recv_len/2)]))
    ...
    #            print('recv data',recv_times,':',np.int16(data_buf_i[0]>>8))
        ...
        for i in range(len(udp_rx_data_bufe)):
            ...
            udp_rx_data.append(
                np.int16(udp_rx_data_bufe[i][1:len(udp_rx_data_bufe[i])]))
...
#                data_longlong.append(np.int64(data_buf_i[:recv_len//8]))
        ...
        for i in range(len(data_longlong)):
            ...
            data_buf_i = np.array(data_longlong[i][1:len(data_longlong[i])])
            data_buf_i = ctypes.cast(
                data_buf_i.ctypes.data, ctypes.POINTER(ctypes.c_longlong))
            udp_rx_data.append(np.int64(data_buf_i[:udp_rx_len[i]]))
        return udp_rx_num, udp_rx_len, udp_rx_data
    #        else:
    #            time.sleep(0.0001)
