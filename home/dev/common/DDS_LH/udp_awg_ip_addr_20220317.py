...
def generate_ip(slot_ip):
    ...
    for s_num in range(len(slot_ip)):
        ...
        ip_len = 0
        ip_cnt = 0
        for i in range(len(slot_ip[s_num][0])):
            
            if (slot_ip[s_num][0][i] == '.' ):
                ip_len += 1
            ip_cnt += 1
            if ip_len == 3:
                break
        
        ip_i = slot_ip[s_num][0][0:ip_cnt]
        ip_sub = int(slot_ip[s_num][0][ip_cnt:len(slot_ip[s_num][0])])
    
        if ip_sub<0 or (ip_sub+(slot_ip[s_num][2]//2)) >255:
            print('err ip')
            return slot_id
