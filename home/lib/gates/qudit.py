import numpy as np
from waveforms.waveform import cosPulse, gaussian, mixing, square

from qlispc import libraries, stdlib
...
def Phase(ctx, qubits, levels, phis):
    ...
    for i, level in enumerate(levels):
        ...
        yield ('!set_phase_ext', level, phi), qubit


def _phase2rfUnitary(ctx, qubits, phis, max_level):
    phis_ave = phis - np.mean(phis)
    for level in range(max_level - 1, 0, -1):
        yield from _rfUnitary(ctx, qubits, np.pi / 2, np.pi / 2, level - 1,
                              level)
        yield from _rfUnitary(ctx, qubits, phis_ave[level] * 2, 0, level - 1,
                              level)
        yield from _rfUnitary(ctx, qubits, np.pi / 2, -np.pi / 2, level - 1,
                              level)
...
def Phase(ctx, qubits, max_level):
    ...
    for level in enumerate(max_level):
        yield ('!set_phase_ext', level, 0), qubit
...
def UU(q, theta, phi, lambda_, delta=0, level1=0, level2=1):
    ...
    if abs(theta) < EPS:
        yield (('Phase', (level1, level2), (delta - (phi + lambda_) / 2,
                                            delta + (phi + lambda_) / 2)), q)
    else:
        ...
        yield (('Phase', (level1, level2), (delta - phi / 2, delta + phi / 2)),
               q)
...
def sxdg(q, level1=0, level2=1):
    yield (('UU', np.pi / 2, -np.pi * 3 / 2, -np.pi / 2, -np.pi, level1,
            level2), q)
    # yield (('UU', np.pi/2, np.pi/2, -np.pi/2, -np.pi/4, level1, level2), q)
