...
def DRAG(t,
    ...
    if shape in ['hanning', 'cosPulse', 'CosPulse']:
        ...
        pulse = (left_edge << (plateau/2)) + square(plateau) + (right_edge >> (plateau/2))
...
@lib.opaque('pi')
def _pi(ctx, qubits, phi=0):
    yield from R(ctx, qubits, phi, 0, 1)


@lib.opaque('pi2')
def _pi2(ctx, qubits, phi=0):
    yield from R(ctx, qubits, phi, 0, 1)
...
def iSWAP(ctx, qubits):
    ...
    pulse = amp1 * coshPulse(width, plateau=plateau, eps=eps) >> (width+plateau) / 2
    yield ('!play', pulse >> (t + before)), ('Z', qubits[0])
    pulse = amp2 * coshPulse(width, plateau=plateau, eps=eps) >> (width+plateau) / 2
    yield ('!play', pulse >> (t + before)), ('Z', qubits[1])

    pulse = ampc * coshPulse(width, plateau=plateau, eps=eps) >> (width+plateau) / 2
